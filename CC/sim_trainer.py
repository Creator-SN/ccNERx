    ...
    def train(self, resume_path=False, resume_step=False, lr1=2e-5, lr2=1e-3, eval_call_epoch=None):
        ...
        for epoch in range(self.num_epochs):
            ...
            for it in train_iter:
                ...
                lstm_outputs, _ = self.birnncrf.__build_features(
                    hidden_states, it['input_ids'].gt(0))
                
                prompt_outputs = self.prompt_model(input_ids=it['prompt_input_ids'], attention_mask=it['prompt_attention_mask'])
                loss = loss.mean()

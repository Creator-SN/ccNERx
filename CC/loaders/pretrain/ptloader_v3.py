...
class PTLoaderV3(IDataLoader):
    ...
    def process_data(self):
        self.myData = PTLoaderV3DataSet(self.train_file,
                                        self.tokenizer,
                                        self.tag_rules,
                                        self.max_seq_length,
                                        self.max_tag_length,
...
class PTLoaderV3DataSet(Dataset):
    def __init__(self,
                 file: str,
    ...
    def convert_embedding(self, item):
        ...
        text_ids = {
            "input_ids": torch.zeros(max_padding, dtype=torch.int),
            "attention_mask": torch.zeros(max_padding, dtype=torch.int),
            "token_type_ids": torch.zeros(max_padding, dtype=torch.int),
            "labels": torch.tensor([-100] * max_padding, dtype=torch.int),
            "origin_labels": None,
        }
        text_ids["input_ids"][:cur_length] = torch.tensor(ids["input_ids"],
                                                          dtype=torch.int)

        text_ids["attention_mask"][:cur_length] = 1

        text_ids["token_type_ids"][cur_length:] = 1

        text_ids["origin_labels"] = text_ids["input_ids"].clone()
        ...
        for i in range(self.max_seq_length):
            ...
            current["input_ids"][cur_length:cur_length +
                                 len(prompt_text)] = torch.tensor(
                                     self.tokenizer.convert_tokens_to_ids(
                                         prompt_text),
                                     dtype=torch.int)

            current["attention_mask"][cur_length:cur_length +
                                      len(prompt_text)] = torch.tensor(
                                          [1] * (len(str(i)) + 1) +
            ...
            mask_label_ids = self.tokenizer.convert_tokens_to_ids(
                list(char_label))
            current["labels"][cur_length:cur_length +
                              len(prompt_text)] = torch.tensor(
        ...
        return tuple(collections[key] for key in self.keys)
    ...
    def __getitem__(self, idx):
        ...
        if isinstance(idx, list):
            ...
            for i, key in enumerate(self.keys):
                data[key] = [self.dataset[j][i] for j in idx]

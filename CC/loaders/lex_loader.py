...
class LXLoader(IDataLoader):
    ...
    def read_data_set(self):
        ...
        for rule in self.tag_rules:
            for label in get_labels(rule, 1):
                self.tag_vocab += label
            for label in get_labels(rule, 3):
                self.tag_vocab += label
...
class LEXBertDataSet(Dataset):
    ...
    def convert_embedding(self, item):
        ...
        if not self.do_predict:
            ...
            input_ids[:len(token_ids)] = torch.tensor(
                token_ids, dtype=torch.long)

            token_type_ids = torch.ones(self.max_seq_length, dtype=torch.long)
            token_type_ids[:text_origin_length] = 0

            attention_mask = torch.ones(self.max_seq_length, dtype=torch.long)
            attention_mask[:len(mask)] = torch.tensor(mask, dtype=torch.long)

            pt_label_ids = torch.zeros(self.max_seq_length, dtype=torch.long)
            pt_label_ids[:len(label_ids)] = torch.tensor(
                label_ids, dtype=torch.long)

            pt_labels = torch.zeros(self.max_seq_length, dtype=torch.long)
            pt_labels[:] = -100
            pt_labels[:len(labels)] = torch.tensor(labels, dtype=torch.long)

            pt_origin_labels = torch.zeros(
                self.max_seq_length, dtype=torch.long)
            pt_origin_labels[:] = -100
            pt_origin_labels[:len(origin_text)] = torch.tensor(
                origin_text, dtype=torch.long)

            return input_ids, token_type_ids, attention_mask, pt_labels, pt_origin_labels, pt_label_ids
        # TODO: predict
    ...
    def __getitem__(self, idx):
        return {
            ...
            'origin_labels': self.origin_labels[idx],
